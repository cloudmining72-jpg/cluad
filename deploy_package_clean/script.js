...
let LIVE_ASSETS = [
  ...
  { id: 'amzn', symbol: 'AMZN', name: 'Amazon.com', type: 'stock', price: 186.50, change: 1.25, icon: 'a', bg: 'linear-gradient(135deg, #ff9900, #ffc04d)', vol: '$29.4M', history: [184, 185, 185.5, 186, 186.5] },
  { id: 'googl', symbol: 'GOOGL', name: 'Alphabet Inc.', type: 'stock', price: 168.90, change: 0.72, icon: 'G', bg: 'linear-gradient(135deg, #4285f4, #34a853)', vol: '$24.6M', history: [166, 167, 168, 167.5, 168.9] }
];
...
function startLiveTickerEngine() {
  setInterval(() => {
    ...
    // 2. Randomly select 2-3 assets to fluctuate realistically
    const assetsToUpdate = [
      LIVE_ASSETS[Math.floor(Math.random() * LIVE_ASSETS.length)],
      LIVE_ASSETS[Math.floor(Math.random() * LIVE_ASSETS.length)]
    ];

    assetsToUpdate.forEach(asset => {
      const deltaPercent = (Math.random() * 0.4 - 0.18); // between -0.18% and +0.22%
      const oldPrice = asset.price;
      asset.price = Number((asset.price * (1 + deltaPercent / 100)).toFixed(asset.price > 100 ? 2 : 4));
      asset.change = Number((asset.change + deltaPercent * 0.5).toFixed(2));
      asset.history.push(asset.price);
      if (asset.history.length > 8) asset.history.shift();

      const isUp = asset.price >= oldPrice;
      const formattedPrice = asset.price > 100 ? asset.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : asset.price.toFixed(4);
      ...
      if (sparkEl) {
        sparkEl.innerHTML = generateSparkline(asset.history, asset.change >= 0);
      }

      // Update Ribbon Tickers
      ...
      // Update Hero mini card if BTC/ETH/SOL
      const miniEl = document.getElementById(`mini-${asset.id}`);
      if (miniEl) {
        miniEl.innerText = `${asset.change >= 0 ? '+' : ''}${asset.change.toFixed(1)}%`;
        miniEl.className = `mono ${asset.change >= 0 ? 'profit' : 'red'}`;
      }
    });
  }, 1600);
}
