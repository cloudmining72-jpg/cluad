...
const sendOtpEmail = async (toEmail, otpCode, type = 'RESET', fullName = '') => {
  ...
  const mailOptions = {
    from: process.env.EMAIL_FROM || '"ClaudeMining" <info@claudemining.com>',
    to: toEmail,
    subject: subject,
    html: `
...
app.get('*', (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.status(404).json({ success: false, message: 'API Server is running. Frontend not built in this directory.' });
  }
});
